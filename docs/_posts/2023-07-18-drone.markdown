...
We mount two volumes coming from our ECS instance to our drone containers: the docker daemon and a data volume to save drone configs in sql. The drone server and drone runner containers requires environment variables to be set for the app to run. In the vars.tf file I’ve specified a map of values. As an aside we can load these variables into SSM and then read the variables by calling data
